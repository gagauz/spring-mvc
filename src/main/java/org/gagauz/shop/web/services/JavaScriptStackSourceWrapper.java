...
public class JavaScriptStackSourceWrapper implements JavaScriptStackSource {

    private class JavaScriptStackWraper implements JavaScriptStack {
        private final String name;
        private final JavaScriptStack original;

        JavaScriptStackWraper(String name, JavaScriptStack original) {
            this.name = name;
        ...
        public List<StylesheetLink> getStylesheets() {
            System.out.println("CSS for " + name);
            original.getStylesheets().stream().forEach(ss -> System.out.println("\t" + ss.getURL()));
    ...
    public JavaScriptStackSourceWrapper(JavaScriptStackSource original) {
        this.original = original;
    }
    ...
    public JavaScriptStack getStack(String name) {
        if (name.equals(InternalConstants.CORE_STACK_NAME)) {
            if (null == coreStackWrapper) {
                coreStackWrapper = new JavaScriptStackWraper(name, original.getStack(name));
            }
