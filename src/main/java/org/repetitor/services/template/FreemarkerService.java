...
public class FreemarkerService {
    ...
    public Mail createPageContent(String name, Map<String, Object> map) {
    ...
    protected String createContentFromFile(final String name,
            Map<String, Object> rootMap) {

        try {

            final Template template = fileCfg.getTemplate(name + BODY_SUFFIX);

            return processTemplate(rootMap, template);
        } catch (Exception e) {
            LOG.error("Exception while merging template " + name, e);
            return createMessageContentFromDB(name, rootMap);
        }
